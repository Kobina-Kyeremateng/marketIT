$(function () {
  ...
  test('should only add mouseenter and mouseleave listeners when not on PhoneNumber', function () {
    var isPhoneNumber     = 'ontouchstart' in document.documentElement
    ...
    $.each(['mouseover', 'mouseout'], function (i, type) {
      strictEqual(type in $._data($template[0], 'events'), !isPhoneNumber, 'does' + (isPhoneNumber ? ' not' : '') + ' listen for ' + type + ' events')
    })
  })
})
