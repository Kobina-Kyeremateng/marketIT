...
if (isset($_POST['saveUser'])) {
 ...
 if(!empty($picture) && !empty($firstName) && !empty($lastName) && !empty($otherName) && 
    ...
    !empty($password) && !empty($regDate) && !empty($service) && !empty($status) &&
    !empty($guardianName) && !empty($guardianMobile) && !empty($guardianRelationship) && !empty($role))
 {
    $dbc = mysqli_connect('localhost', 'root', '','marketit') or die('Error connecting to mysql server');
    $query = "INSERT INTO users(FirstName,LastName,OtherNames,DOB,Gender,PrefixID,ReligionID,NationalityID,Mobile,Address,Email, Password,Image,RoleID,StatusID,regDate, serviceID,  guardianName, guardianMobile, guardianRelationship)VALUES('$firstName','$lastName','$otherName','$dob','$gender','$title','$religion','$nationality','$mobile','$address','$email','$password','$picture','$role','$status','$regDate','$service','$guardianName','$guardianMobile', '$guardianRelationship')";
    $query2 = "INSERT INTO login(Email,Password,RoleID,Image)VALUES('$email','$password','$role','$picture')";
